...
export default function LossLandscape({ onResult, resetSignal }: ModuleComponentProps) {
  ...
  const bestLoss = useMemo(() => mse(dataset.points, best.m, best.b), [dataset, best]);
  ...
  return (
    <div className="panel">
      ...
      <div className="ctxGrid" style={{ gridTemplateColumns: "1.1fr 1fr", alignItems: "start" }}>
        ...
        <div className="vizStage" style={{ padding: 12 }}>
          <div className="panelTitle" style={{ paddingLeft: 6 }}>
            Parameter space: every pixel is one possible line
          </div>
          <LossContour
      ...
      <div className="controlRow" style={{ marginTop: 12 }}>
        ...
        {inValley && <span className="copiedFlash">🏞 You are in the valley!</span>}
      </div>

      <p className="hintText" style={{ marginTop: 10 }}>
        Drag the blue dot on the map: green is the valley (low loss), orange is high ground. Each
        position of the dot IS a line on the left. The star is the best possible line.
      </p>
      ...
      <ChallengeCards
        challenges={[
          ...
          },
          {
            id: "bad-on-purpose",
            title: "Sabotage",
            goal: "Move to the worst corner on purpose. What does the line on the left look like?"
          },
          {
            id: "compare",
